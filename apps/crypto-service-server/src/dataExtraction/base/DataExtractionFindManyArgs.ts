/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DataExtractionWhereInput } from "./DataExtractionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DataExtractionOrderByInput } from "./DataExtractionOrderByInput";

@ArgsType()
class DataExtractionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DataExtractionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DataExtractionWhereInput, { nullable: true })
  @Type(() => DataExtractionWhereInput)
  where?: DataExtractionWhereInput;

  @ApiProperty({
    required: false,
    type: [DataExtractionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DataExtractionOrderByInput], { nullable: true })
  @Type(() => DataExtractionOrderByInput)
  orderBy?: Array<DataExtractionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DataExtractionFindManyArgs as DataExtractionFindManyArgs };
